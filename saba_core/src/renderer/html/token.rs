...
pub struct HtmlTokenizer {
    state: State,                    // ステートマシーンの現在の状態
    pos: usize,                      // ステートマシーンの現在の位置
    reconsume: bool,                 // 現在のトークンを再利用するかどうか
    latest_token: Option<HtmlToken>, // 最新のトークン
    input: Vec<char>,                // 入力文字列の文字のベクター
    buf: String,                     // バッファ
}
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        loop {
            ...
            match self.state {
                ...
                State::AttributeValueDoubleQuote => {
                    if c == '"' {
                ...
                State::AttributeValueSingleQuote => {
                    if c == '\'' {
                        self.state = State::AfterAttributeValueQuoted;
                        continue;
                    }
...
pub enum HtmlToken {
    ...
    StartTag {
        tag: String,                // タグの名前
        self_closing: bool,         // 自己閉じタグかどうか
        attributes: Vec<Attribute>, // 属性のベクター
    },
    // 終了タグ
    EndTag {
        tag: String,
...
pub enum State {
    ...
    /// https://html.spec.whatwg.org/multipage/parsing.html#attribute-value-double-quote-state
    AttributeValueDoubleQuote,
    /// https://html.spec.whatwg.org/multipage/parsing.html#attribute-value-single-quote-state
    AttributeValueSingleQuote,
    /// https://html.spec.whatwg.org/multipage/parsing.html#attribute-value-unquoted-state
    AttributeValueUnquoted,
    /// https://html.spec.whatwg.org/multipage/parsing.html#after-attribute-value-state
    ...
    /// https://html.spec.whatwg.org/multipage/parsing.html#temporary-buffer-state
    TemporaryBuffer,
}
