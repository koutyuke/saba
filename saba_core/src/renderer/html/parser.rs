...
impl HtmlParser {
                ...
                InsertionMode::InHead => {
                    match token {
                        ...
                        Some(HtmlToken::StartTag {
                            ...
                            // 仕様に準拠していないが、headがないHTMLの場合の無限ループ回避
                            if tag == "head" {
    ...
    /// 新しい要素を `stack_of_open_elements` にある現在(最後)の要素の子要素として挿入する
    fn insert_element(&mut self, tag: &str, attributes: Vec<Attribute>) {
        ...
        if current.borrow().first_child().is_some() {
            ...
            node.borrow_mut()
                .set_previous_sibling(Rc::downgrade(&last_sibling.expect("last_sibling should be Some")));
        } else {
    ...
    /// `stack_of_open_elements` スタックから現在のノードを削除し、削除したかどうかを bool で返す
    ...
    /// `stack_of_open_elements` スタックから特定の種類の要素が現れるまでノードを取り出し続ける関数
    ///  閉じたタグの整合性がない場合でも無視してパースできるようにする
    ...
    /// 引数の文字を現在のノードが文字ノードなら追加、そうでないなら文字ノードを作成する
    fn insert_char(&mut self, c: char) {
        let current = match self.stack_of_open_elements.last() {
            Some(n) => n.clone(),

            // ここに到達する場合、rootノードの中に文字を入れることになるため仕様上不適切であり、無視する
            None => return,
        };
