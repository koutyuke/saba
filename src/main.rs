...
fn handle_url(url: String) -> Result<HttpResponse, Error> {
    ...
    let parsed_url = match Url::new(url.to_string()).parse() {
        Ok(url) => url,
        Err(e) => {
            return Err(Error::UnexpectedInput(format!("input html is not supported: {:?}", e)));
        }
    };
    ...
    let response = match client.get(
        ...
        parsed_url
            .port()
            .parse::<u16>()
            .expect(&format!("port number should be u16 but got {}", parsed_url.port())),
        parsed_url.path(),
    ) {
        Ok(res) => {
        ...
        Err(e) => return Err(Error::Network(format!("failed to get http response: {:?}", e))),
    };
    Ok(response)
}
