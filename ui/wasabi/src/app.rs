...
impl WasabiUI {
    ...
    fn handle_mouse_input(&mut self, handle_url: fn(String) -> Result<HttpResponse, Error>) -> Result<(), Error> {
        if let Some(MouseEvent { button, position }) = Api::get_mouse_cursor_info() {
            ...
            if button.l() || button.c() || button.r() {
                // 相対位置を計算する
                let relative_pos = (position.x - WINDOW_INIT_X_POS, position.y - WINDOW_INIT_Y_POS);
                ...
                let position_in_content_area = (relative_pos.0, relative_pos.1 - TITLE_BAR_HEIGHT - TOOLBAR_HEIGHT);
    ...
    fn setup_toolbar(&mut self) -> OsResult<()> {
        ...
        self.window
            .draw_line(DARKGREY, 0, TOOLBAR_HEIGHT + 1, WINDOW_WIDTH - 1, TOOLBAR_HEIGHT + 1)?;

        // アドレスバーの横に"Address:"という文字列を描画
        self.window
            .draw_string(BLACK, 5, 5, "Address:", StringSize::Medium, /*underline=*/ false)?;
    ...
    fn update_address_bar(&mut self) -> Result<(), Error> {
        ...
        if self
            ...
            .is_err()
        {
            return Err(Error::InvalidUI("failed to clear an address bar".to_string()));
        }
        ...
        if self
            ...
            .is_err()
        {
            return Err(Error::InvalidUI("failed to update an address bar".to_string()));
        }
    ...
    fn clear_content_area(&mut self) -> Result<(), Error> {
        ...
        if self
            ...
            .is_err()
        {
            return Err(Error::InvalidUI("failed to clear a content area".to_string()));
        }
